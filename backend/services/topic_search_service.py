...
"""
    ...
    if openrouter_key:
        configured_model = getattr(settings, "openrouter_model", "") or os.getenv("OPENROUTER_MODEL", "openrouter/auto")
        selected_model = model_choice.lower().strip()
        model_id = MODEL_MAP.get(selected_model, model_choice)
        ...
        async with httpx.AsyncClient(timeout=45.0) as client:
            ...
            if res.status_code == 200:
                ...
                print(
                    f"[TopicSearch] OpenRouter returned HTTP {res.status_code}: {res.text}. Using fallback LLM...",
                    flush=True,
                )
